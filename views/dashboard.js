...
export function render(container, { state }) {
  ...
  const metrics = [
    { label: "业务域", value: `${summary.businessLineCount}条`, hint: "K2完整模拟，E1待接入" },
    { label: "在线策略", value: `${summary.onlineAssetCount}/${summary.assetCount}`, hint: `${summary.strategyHealthRate}%上线率` },
    { label: "可复用资产", value: `${summary.reusableAssetCount}个`, hint: "支持跨级别复制和差异配置" },
    { label: "下发完成", value: `${summary.completedBatchRate}%`, hint: "按策略批次追踪" },
    { label: "产研依赖", value: `${summary.dataRequirementCount}项`, hint: "字段/事件/刷新/验收" }
  ];

  container.innerHTML = `<section class="page-header" aria-label="策略总控"><div><p class="section-kicker">策略团队经营视角</p><h1>英语策略运营总控台</h1><p>用同一套后台管理各级别的策略覆盖、资产配置、人群圈选、下发追踪与效果复盘。当前用K2现有策略做完整模拟，E1用已找到产品/QA资料做待接入样例。</p></div>${renderBadge("info", "K2模拟样例")}</section>${renderMetricStrip(metrics)}<section class="panel"><header class="panel__header"><div><p class="section-kicker">业务覆盖</p><h2>策略健康矩阵</h2></div></header>${renderTable({ columns: [
    { key: "name", label: "业务域" },
    { key: "sampleDepth", label: "样本深度", format: (value) => value === "full" ? "完整模拟" : "待接入样例" },
